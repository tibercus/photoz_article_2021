...
\begin{table*}
    ...
    \hline
        ...
        Pan-STARRS1 DR2 & $g_{PS}$,$r_{PS}$,$i_{PS}$,$z_{PS}$,$y_{PS}$ & PSF, Kron & 30000 (3/4 of the sky) & \\
        DESI LIS DR8 & $g_{LS}$,$r_{LS}$,$z_{LS}$ & Model & 14000 (1/3 of the sky) & \\
        WISE & w1,w2 & forced photometry & whole sky & \\[1ex]
        \hline
    \end{tabular}
\end{table*}
...
\begin{table*}
    ...
    \hline
        \refstepcounter{ModelNumber}\theModelNumber\label{model:pw} & & + & & Burenin et al. (2021) & \ref{feats:ps-mags-1}, \ref{feats:ps-mags-2}, \ref{feats:ps-colors-1}, \ref{feats:ps-colors-2}, \ref{feats:wise-mags-1}, \ref{feats:wise-colors-1}, \ref{feats:ps-wise-colors-1} \\ %Pan-STARRS mags (1 and 2) and colors (1 and 2), WISE mags and colors, Pan-STARRS + WISE colors \\
        \refstepcounter{ModelNumber}\theModelNumber\label{model:pdw} & & + & + & \cite{2019AJ....157..168D} & \ref{feats:ps-mags-1}, \ref{feats:ps-colors-1}, \ref{feats:ls-mags-1}, \ref{feats:ls-colors-1}, \ref{feats:wise-mags-1}, \ref{feats:wise-colors-1}, \ref{feats:ps-ls-colors-1}, \ref{feats:ps-wise-colors-1} \\ % Pan-STARRS mags (1) and colors (1), DESI LIS mags and colors, WISE mags and colors, Pan-STARRS + DESI LIS colors, Pan-STARRS + WISE colors \\
        \refstepcounter{ModelNumber}\theModelNumber\label{model:dw} & & & + & \cite{2019AJ....157..168D} & \ref{feats:ls-mags-1}, \ref{feats:ls-colors-1}, \ref{feats:wise-mags-1}, \ref{feats:wise-colors-1}, \ref{feats:ls-wise-colors-1} \\ % DESI LIS mags and colors, WISE mags and colors, DESI LIS + WISE colors \\
        \refstepcounter{ModelNumber}\theModelNumber\label{model:spw} & + & + & & Burenin et al. (2021) & \ref{feats:sdss-mags-1}, \ref{feats:sdss-mags-2}, \ref{feats:ps-colors-1}, \ref{feats:ps-colors-2}, \ref{feats:wise-mags-1}, \ref{feats:wise-colors-1}, \ref{feats:sdss-wise-colors-1}, \ref{feats:ps-wise-colors-1} \\
        \refstepcounter{ModelNumber}\theModelNumber\label{model:spdw} & + & + & + & \cite{2019AJ....157..168D} & \ref{feats:sdss-mags-1}, \ref{feats:sdss-colors-1}, \ref{feats:ps-mags-1}, \ref{feats:ps-colors-1}, \ref{feats:ls-mags-1}, \ref{feats:ls-colors-1}, \ref{feats:wise-mags-1}, \ref{feats:wise-colors-1}, \ref{feats:sdss-wise-colors-1}, \ref{feats:sdss-ls-colors-1}, \ref{feats:ps-wise-colors-1} \\ % SDSS mags and colors, Pan-STARRS mags (1) and colors (1), DESI LIS mags and colors, WISE mags and colors, SDSS + WISE colors, SDSS + DESI LIS colors, Pan-STARRS + WISE colors \\
    \hline
    \end{tabular}
\end{table*}
...
Нашей целью является многосторонний анализ поведения моделей, построенных на данных разных фотометрических обзоров (SDSS, Pan-STARRS1, DESI Legacy Surveys, WISE). В разделе \ref{ssec:results-s82x} модели тестируются на данных рентгеновской выборки Stripe82X, оценивается точность для рентгеновских данных различной глубины, в сравнении с шаблонными и нейросетевыми моделями photo-z рентгеновских источников рассматриваемыми в литературе (Ananna, Brescia). В следующем разделе \ref{ssec:results-cv2-and-dr16q} проводится анализ поведения моделей на выборках большого объема в зависимости от разных факторов (оптическая яркость, красное смещение, спектральный класс и фотометрический класс объектов). В разделе \ref{ssec:results-zconf} проводится анализ оценки уверенности прогнозов zConf. В последнем разделе \ref{ssec:results-prec-comp} оценивается точность и полнота отбора объектов для разных порогов уверенности zConf.

\subsection{Анализ качества моделей на рентгеновских данных (Stripe82X)}\label{ssec:results-s82x}

Этот раздел посвящен анализу моделей на рентгеновской выборке Stripe82X. Это важно для понимания точности моделей на реальных данных. Модели сравниваются по двум критериям -- точность (метрики NMAD и доля выбросов) и калибровка доверительного интервала 68\% уровня доверия. Разные модели имеют разное покрытие неба, поэтому важно понимать, в каких областях неба какая точность может быть достигнута. Для такого сравнения будет использоваться общее подмножество объектов Stripe82X для которых есть фотометрия всех трех используемых обзоров.

В таблице \ref{tab:stripe82x-intersection} представлено сравнение моделей \ref{model:pw} -- \ref{model:spdw} на общем подмножестве объектов для трех порогов по рентгеновскому потоку: $F_{X,0.5-2}>3\cdot10^{-15}$\,эрг/с/см$^2$ (соответствуют глубоким наблюдениям SRG/eROSITA поля Lockman Hole во время PV-фазы), $F_{X,0.5-2}>3\cdot10^{-14}$\,эрг/с/см$^2$ (соответствуют глубоким наблюдениям SRG/eROSITA поля eFETS во время PV-фазы, а также ожидаемой чувствительности 4-х летнего обзора всего неба SRG/eROSITA в районе небесного экватора), $F_{X,0.5-2}>4\cdot10^{-14}$\,эрг/с/см$^2$ (соответствуют чувствительности первого годового обзора всего неба SRG/eROSITA). 

Как можно видеть (см. таблицу \ref{tab:stripe82x-intersection}), точность всех моделей \ref{model:pw} -- \ref{model:spdw} ухудшается для более глубоких рентгеновских данных (по метрике NMAD примерно до 1.5 раз и в 1.5 -- 2 раза по доле катастрофических выбросов). Лучшую точность при прямом сравнении показывает модель \ref{model:spdw}, основанная на данных всех трех используемых обзоров. Наиболее интересует модель \ref{model:pw}, которая может применяться на всем российском небе eRosita. Эта модель показывает точность при прямом сравнении в 1.5 -- 2 раза хуже наиболее точной модели. Модель \ref{model:dw}, основанная только на самом глубоком из используемых обзоров, то есть DESI LIS, и имеет посредственную точность по NMAD и доле выбросов, но интересно, что относительное изменение точности при добавлении более глубоких рентгеновских данных у этой модели меньше, чем для других моделей -- точность по метрике NMAD не меняется, а для выбросов увеличивается на 9 -- 13\% при последовательном добавлении более глубоких рентгеновских данных в тестирование.

Эта же модель показывает калибровку 68\% доверительных интервалов лучше других моделей: модуль отклонения $C_{68} - 0.68$ теоретического уровня доверия от фактического составляет 0.009 на объектах с потоком $FSoft \geq 3e-15$, и увеличивается до 0.057 на подмножествах более ярких в рентгене объектов. Модель на всем небе \ref{model:pw} так же показывает сравнительно хорошую калибровку -- наибольшее отклонение проявляется на объектах с потоком $FSoft \geq 1e-14$ и составляет 0.035. Более точные по NMAD и доле выбросов демонстрируют излишнюю уверенность прогнозов -- у большей доли объектов истинное значение попадает внутрь спрогнозированного интервала. При этом, чем точнее модель, тем большую уверенность демонстрируют доверительные интервалы -- наиболее точная модель демонстрирует уровень доверия 80\% вместо необходимых 68\% ($C_{68} - 0.68 = 0.113, 0.12, 0.122$ для порогов по потоку $FSoft \geq 3e-15, 1e-14, 4e-14$, соответственно).
...
Другим важным аспектом этой работы является сравнение с моделями SOTA (Ananna, Brescia), поскольку при проведении исследований мы ориентировались на точность этих моделей. Это сравнение можно проводить напрямую, как в таблице \ref{tab:stripe82x-intersection}, что важно для понимания, где находится, в смысле точности, каждая из представленных моделей относительно SOTA. Лучшая из представленных моделей \ref{model:spdw} показывает точность почти в 2 раза лучше по метрикам NMAD и доля катастрофических выбросов, чем модели SOTA (на объектах  с рентгеновским потоком $FSoft \geq 3e-15$, $NMAD, n_{>0.15} = 0.033, 0.083~/~0.064, 0.171~/~0.064, 0.157$ для моделей \ref{model:spdw} / Ananna / Brescia, соответственно). При этом модель на всем небе \ref{model:pw} показывает точность немного лучше, но сопоставимую с SOTA ($NMAD, n_{>0.15} = 0.056, 0.145$).

Недостатком такого сравнения является значительное сужение выборки, обусловленное отсутствием фотометрии какого-либо из используемых каталогов для некоторых объектов. Для проведения более честного сравнения точности -- при полноте прогнозов как у моделей SOTA -- будет использоваться <<комбинированная модель>>, то есть если для объекта невозможно сделать прогноз наиболее точной моделью, будет взят прогноз другой менее точной модели (порядке приоритета -- \ref{model:dw} и \ref{model:pw}). Результаты для объектов Stripe82X с надежным оптическим компаньоном ($QF < 2.1$ проверить!!!) представлены в таблице \ref{tab:stripe82x-combined}. Разница в точности составляет почти 1.5 раза по метрикам NMAD и для выбросов (для объектов $FSoft \geq 3e-15$ получены значения метрик $FSoft \geq 3e-15$, $NMAD, n_{>0.15} = 0.038, 0.101~/~0.065, 0.172~/~0.066, 0.156$). Видно, что метрики моделей Ananna и Brescia не изменились существенно, по сравнению с прямым сравнением моделей (табл. \ref{tab:stripe82x-intersection}), а комбинированный прогноз показывает на 0.18 бо'льшую долю выбросов по сравнению с точностью лучшей модели. Это связано с более низкой точностью прогнозов моделей \ref{model:dw} и \ref{model:pw}.
...
Так же, по опыту предыдущий исследований, точность на разных классах объектов (галактики/квазары, протяженные/точечные) может значительно отличаться, поэтому важно понимать поведение моделей на этих классах. Результаты по этому направлению представлены в таблицах \ref{tab:stripe82x-spec-class} и \ref{tab:stripe82x-phot-class}) для спектральных классов галактика/квазар и фотометрических классов протяженный/точечный, соответственно. Заметим, что не для всех объектов Stripe82X имеется классификация по спектру, поэтому, при анализе на спектральных классах, использовались только объекты с имеющейся классификацией галактика/квазар.

На спектральных галактиках комбинированный прогноз показывает бо'льшую долю выбросов по сравнению с SOTA, при этом, превосходит по NMAD. На квазарах комбинированный прогноз превосходит SOTA почти в 2 раза по обеим метрикам точности. На оптических классах объектов (точечные и протяженные) комбинированные прогнозы показывают существенно лучшую точность по обеим метрикам. При этом прогнозы на точечных источниках показывают бо'льшую долю выбросов по сравнению с прогнозами протяженных объектов. 

\begin{table*}
	\begin{tabular}{|l|lllr|lllr|lllr|}
            \hline
    Model   & \multicolumn{4}{c|}{$F_{X,0.5-2}\geq 3e-15$} & \multicolumn{4}{c|}{$F_{X,0.5-2} \geq 1e-14$} & \multicolumn{4}{c|}{$F_{X,0.5-2} \geq 4e-14$} \\
                &             $NMAD$ &        $n_{>0.15}$ &  $C_{68} - 0.68$ & $N_{preds}$ &             $NMAD$ &        $n_{>0.15}$ &  $C_{68} - 0.68$ & $N_{preds}$ &             $NMAD$ &        $n_{>0.15}$ &  $C_{68} - 0.68$ & $N_{preds}$ \\
            \hline
            \multicolumn{13}{c}{Spectral class = GALAXY}\\
            \hline
            \ref{model:pw} &              0.048 &           0.117 &  \textbf{-0.018} &         420 &               0.05 &           0.096 &           -0.023 &         178 &              0.081 &           0.071 &           -0.323 &          14 \\
            ...
            \hline
            \multicolumn{13}{c}{Spectral class = QSO}\\
            \hline
            \ref{model:pw} &              0.061 &           0.158 &            0.037 &        1408 &               0.05 &           0.121 &            0.042 &         899 &              0.033 &           0.075 &            0.053 &         161 \\
            ...
            \hline
            \multicolumn{13}{c}{Photometric class = Extended}\\
            \hline
            \ref{model:pw} &              0.046 &           0.081 &   \textbf{0.017} &         664 &              0.043 &           0.072 &  \textbf{0.013} &         349 &              0.042 &           0.078 &           -0.071 &          64 \\
            ...
            \hline
            \multicolumn{13}{c}{Photometric class = Point-like}\\     \hline                
            \ref{model:pw} &              0.062 &           0.175 &            0.033 &        1528 &              0.054 &           0.136 &           0.043 &         935 &              0.035 &           0.102 &            0.068 &         147 \\
            ...
            \hline
            \end{tabular}
            \caption{Точность и калибровка моделей photo-z на разных спектральных и фотометрических классах объектов в поле Stripe82X}\label{tab:stripe82x-phot-class}
\end{table*}
...
На графиках \ref{fig:metrics-dr16q} и \ref{fig:metrics-cv2-total} показаны зависимости метрик точности NMAD и доля выбросов в зависимости от спектрального красного смещения, прогноза красного смещения и оптической яркости объектов. Видно, что модели показывают разное качество прогнозов на разных красных смещениях. Наблюдаются ухудшения точности в районе $spec-z \sim 1.0$ для всех моделей, $spec-z \sim 1.7$ для модели \ref{model:dw} и $spec-z \sim 3.0$ для моделей, не использующих данные SDSS, \ref{model:dw}, \ref{model:pdw}, \ref{model:pw}. %. Так же можно видеть, что близкие квазары ($spec-z < 1$) дают существенную долю выбросов по сравнению с галактиками 
...
\begin{figure*}
    \centering
    \includegraphics[width=0.99\linewidth]{images/metrics-zconf-cv2-log_zspec.png}
...
\begin{figure*}
    \centering
    \includegraphics[width=0.99\linewidth]{images/class-extended-cv2.png}
    \caption{Метрики на ярких и слабых протяженных источниках на кросс-валидации в зависимости от spec-z}
    \label{fig:class-galaxy-cv2}
\end{figure*}

\begin{figure*}
    \centering
    \includegraphics[width=0.99\linewidth]{images/class-pointlike-cv2.png}
    \caption{Метрики на ярких и слабых точечных источниках на кросс-валидации в зависимости от spec-z}
    \label{fig:class-galaxy-cv2}
\end{figure*}
...
\begin{figure*}
    \centering
    \includegraphics[width=0.99\linewidth]{images/metrics-zconf-cv2-gal-log_zspec.png}
    \caption{Метрики в зависимости от zConf (кросс-валидация галактики, дифференциальная по zConf)}
    \label{fig:metrics-zconf-cv2}
\end{figure*}
...
\begin{figure*}
    \centering
    \includegraphics[width=0.99\linewidth]{images/metrics-zconf-cv2-qso-log_zspec.png}
    \caption{Метрики в зависимости от zConf (кросс-валидация квазары, дифференциальная по zConf)}
    \label{fig:metrics-zconf-cv2}
\end{figure*}
...
\begin{figure*}
    \centering
    \includegraphics[width=0.99\linewidth]{images/metrics-zconf-dr16q-log_zspec.png}
    \caption{Метрики в зависимости от zConf (DR16q, дифференциальная по zConf)}
    \label{fig:metrics-zconf-cv2}
\end{figure*}
...
\begin{figure*}
    \centering
    \includegraphics[width=0.99\linewidth]{images/metrics-zconf-cv2-log-cum_zspec.png}
    \caption{Метрики в зависимости от zConf (кросс-валидация, кумулятивная по zConf)}
    \label{fig:metrics-zconf-cv2}
\end{figure*}
...
\begin{figure*}
    \centering
    \includegraphics[width=0.99\linewidth]{images/metrics-zconf-cv2-gal-log-cum_zspec.png}
    \caption{Метрики в зависимости от zConf (кросс-валидация галактики, кумулятивная по zConf)}
    \label{fig:metrics-zconf-cv2}
\end{figure*}
...
\begin{figure*}
    \centering
    \includegraphics[width=0.99\linewidth]{images/metrics-zconf-cv2-qso-log-cum_zspec.png}
    \caption{Метрики в зависимости от zConf (кросс-валидация квазары, кумулятивная по zConf)}
    \label{fig:metrics-zconf-cv2}
\end{figure*}
...
\begin{figure*}
    \centering
    \includegraphics[width=0.99\linewidth]{images/metrics-zconf-dr16q-log-cum_zspec.png}
    \caption{Метрики в зависимости от zConf (DR16q, кумулятивная по zConf)}
    \label{fig:metrics-zconf-cv2}
\end{figure*}
...
\begin{figure*}
    \centering
    \includegraphics[width=0.9\linewidth]{images/zconf-by-spec_z-x-n015.png}
    \caption{Выбор порога zConf по spec-z и порогу по доле выбросов.}
    \label{fig:my_label}
\end{figure*}

\begin{figure*}
    \centering
    \includegraphics[width=0.9\linewidth]{images/zconf-by-spec_z-x-n015_zl20.png}
    \caption{Выбор порога zConf по spec-z и порогу по доле выбросов ($z_{mag} \leq 20$).}
    \label{fig:my_label}
\end{figure*}

\begin{figure*}
    \centering
    \includegraphics[width=0.9\linewidth]{images/zconf-by-spec_z-x-n015_zgr20.png}
    \caption{Выбор порога zConf по spec-z и порогу по доле выбросов ($20 < z_{mag} \leq 22$).}
    \label{fig:my_label}
\end{figure*}
