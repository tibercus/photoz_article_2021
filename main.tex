...
\begin{enumerate}
	\item Цель -- оценить точности прогнозов на полугодовом, годовом и четырехлетнем обзрах ерозиты
	\item Точность на объектах 4e-14
	\item Точность на объектах 1e-14
	\item Точность на объектах 3e-15
	\item Везде превосходство модели 35
	\item Сравнение с Ananna, Brescia
\end{enumerate}

\subsection{Поведение моделей на объектах разных популяциях объектов}
\begin{enumerate}
	\item Описание цели
	\item На разных красных смещениях модели имеют разнородную точность
	\item Точность моделей ухудшается на слабых источниках
	\item Модели дают высокую точность для галактик и значительное число ошибок для близких квазаров
	\item Точность на ярких и слабых галактиках
	\item Точность на ярких и слабых квазарах
\end{enumerate}

\subsection{Стратегия отбора надежных прогнозов}
\begin{enumerate}
	\item Что мы хотим получить?
	\item Смотрим на графики и понимаем, что zConf > 0.4 == шлак
\end{enumerate}

\subsection{Точность и полнота прогнозов}
\begin{enumerate}
	\item Что хотим, зачем нужно
	\item Метрики определяем в соответствии со стратегией отбора
	\item полнота
	\item точность
\end{enumerate}
